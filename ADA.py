...
ada_df.set_index('timestamp_start', inplace=True)
ada_df.head()

# %%
ada = pd.read_csv('full_ADAUSDT_1m.csv')

ada.columns = col_names
ada['timestamp_start'] = pd.to_datetime(ada['timestamp_start'], unit='ms')
ada.set_index('timestamp_start', inplace=True)
ada['log_return'] = np.log(ada['close'] / ada['close'].shift(1))
ada.head()
...
start_time = datetime(2020, 1, 19, 9, 10, 0)
...
# %%
ada_df['log_return'] = np.log(ada_df['close'] / ada_df['close'].shift(1))
...
# %%
# POS 
ada_timestamps = ada_df.index.values  # Get the timestamp_start values as an array

valid_ranges = []
for start, end in pos_ts1_5_arr:
    # Filter rows within the range
    filtered_range = ada_df.loc[start:end, 'log_return']
...
ada_1min = ada.resample('5T').sum()

# Print the resampled DataFrame
total_log_r = ada_1min['log_return']
total_log_r
...
plt.title('ADA/USDT Histograms of log returns (Jump Log and Total Log)')
...
ada_ts_btcj_start_dates = ada_ts_btcj_start.date

# Create a DataFrame to count the frequency of timestamps for each day
date_counts = pd.Series(ada_ts_btcj_start_dates).value_counts().sort_index()

# Generate a complete date range from the minimum to the maximum date
date_range = pd.date_range(start=ada_ts_btcj_start.min().date(), end=ada_ts_btcj_start.max().date(), freq='D')

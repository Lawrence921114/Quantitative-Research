...
bch_df.set_index('timestamp_start', inplace=True)
bch_df.head()

# %%
bch = pd.read_csv('full_BCHUSDT_1m.csv')

bch.columns = col_names
bch['timestamp_start'] = pd.to_datetime(bch['timestamp_start'], unit='ms')
bch.set_index('timestamp_start', inplace=True)
bch['log_return'] = np.log(bch['close'] / bch['close'].shift(1))
bch.head()
...
start_time = datetime(2019, 12, 31, 0, 1, 0)
...
# %%
bch_df['log_return'] = np.log(bch_df['close'] / bch_df['close'].shift(1))
...
# %%
# POS 
bch_timestamps = bch_df.index.values  # Get the timestamp_start values as an array

valid_ranges = []
for start, end in pos_ts1_5_arr:
    # Filter rows within the range
    filtered_range = bch_df.loc[start:end, 'log_return']
...
bch_1min = bch.resample('5T').sum()

# Print the resampled DataFrame
total_log_r = bch_1min['log_return']
total_log_r
...
plt.title('BCH/USDT Histograms of log returns (Jump Log and Total Log)')
...
bch_ts_btcj_start_dates = bch_ts_btcj_start.date

# Create a DataFrame to count the frequency of timestamps for each day
date_counts = pd.Series(bch_ts_btcj_start_dates).value_counts().sort_index()

# Generate a complete date range from the minimum to the maximum date
date_range = pd.date_range(start=bch_ts_btcj_start.min().date(), end=bch_ts_btcj_start.max().date(), freq='D')
...
# Customize the plot
plt.title('Frequency of Timestamps Per Day (LTC)')
plt.xlabel('Date')
plt.ylabel('Frequency')

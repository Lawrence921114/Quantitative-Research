...
ltc_df.set_index('timestamp_start', inplace=True)
ltc_df.head()

# %%
ltc = pd.read_csv('full_LTCUSDT_1m.csv')

ltc.columns = col_names
ltc['timestamp_start'] = pd.to_datetime(ltc['timestamp_start'], unit='ms')
ltc.set_index('timestamp_start', inplace=True)
...
start_time = datetime(2020, 1, 9, 8, 10, 0)
...
# %%
ltc_df['log_return'] = np.log(ltc_df['close'] / ltc_df['close'].shift(1))
...
# %%
# POS 
ltc_timestamps = ltc_df.index.values  # Get the timestamp_start values as an array

valid_ranges = []
for start, end in pos_ts1_5_arr:
    # Filter rows within the range
    filtered_range = ltc_df.loc[start:end, 'log_return']
...
ltc_1min = ltc.resample('5T').sum()

# Print the resampled DataFrame
total_log_r = ltc_1min['log_return']
total_log_r
...
plt.title('LTC/USDT Histograms of log returns (Jump Log and Total Log)')
...
ltc_ts_btcj_start_dates = ltc_ts_btcj_start.date

# Create a DataFrame to count the frequency of timestamps for each day
date_counts = pd.Series(ltc_ts_btcj_start_dates).value_counts().sort_index()

# Generate a complete date range from the minimum to the maximum date
date_range = pd.date_range(start=ltc_ts_btcj_start.min().date(), end=ltc_ts_btcj_start.max().date(), freq='D')

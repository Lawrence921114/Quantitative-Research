...
near_df.set_index('timestamp_start', inplace=True)
near_df.head()

# %%
near = pd.read_csv('full_NEARUSDT_1m.csv')

near.columns = col_names
near['timestamp_start'] = pd.to_datetime(near['timestamp_start'], unit='ms')
near.set_index('timestamp_start', inplace=True)
near['log_return'] = np.log(near['close'] / near['close'].shift(1))
near.head()
...
start_time = datetime(2020, 10, 15, 8, 1, 0)
...
# %%
near_df['log_return'] = np.log(near_df['close'] / near_df['close'].shift(1))
...
# %%
# POS 
near_timestamps = near_df.index.values  # Get the timestamp_start values as an array

valid_ranges = []
for start, end in pos_ts1_5_arr:
    # Filter rows within the range
    filtered_range = near_df.loc[start:end, 'log_return']
...
# %%
near.index = pd.to_datetime(near.index)

# Resample the data to 5-minute intervals and sum all values
near_1min = near.resample('5T').sum()

# Print the resampled DataFrame
total_log_r = near_1min['log_return']
total_log_r
...
plt.title('NEAR/USDT Histograms of log returns (Jump Log and Total Log)')
...
near_ts_btcj_start_dates = near_ts_btcj_start.date

# Create a DataFrame to count the frequency of timestamps for each day
date_counts = pd.Series(near_ts_btcj_start_dates).value_counts().sort_index()

# Generate a complete date range from the minimum to the maximum date
date_range = pd.date_range(start=near_ts_btcj_start.min().date(), end=near_ts_btcj_start.max().date(), freq='D')
